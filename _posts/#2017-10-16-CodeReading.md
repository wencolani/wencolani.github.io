...
# models
models文件夹下主要含有一下两个文件，将model的调用和model的定义分开：
...
class Module(chainer.Chain):
	...
	def __call__(self, x,is_train):
		if self.dropout_rate!=0:
			x = F.dropout(x,ratio=self.dropout_rate,train=is_train)
		z = self.x2z(x)
		if self.is_batchnorm:
			z = self.bn(z,test=not is_train)
...
class Block(chainer.Chain):
	...
	def __call__(self,x,is_train):
		for name, _ in self.forward:
			x = getattr(self,name)(x,is_train)
		return x
...
class Tunnel(chainer.Chain):
	...
	def easy_case(self,x,neighbor_entities,neighbor_dict,assign,entities,relations,is_train):
	...
	"""
	def __call__(self,x,neighbor_entities,neighbor_dict,assign,entities,relations,is_train):
		if self.layer==0:
			return self.easy_case(x,neighbor_entities,neighbor_dict,assign,entities,relations,is_train)
		...
		for r in bundle:
			...
			if len(rx)==1:
				rx=rx[0]
				if r%2==0:	rx = getattr(self,self.forwardH[r//2][0])(rx,is_train)
				else:		rx = getattr(self,self.forwardT[r//2][0])(rx,is_train)
...
class Model(chainer.Chain):
	...
	def get_context(self,entities,links,relations,edges,order,xp,is_train):
		if self.depth==order:
			return self.embedE(xp.array_int(entities,is_train))
		...
		for i,e in enumerate(entities):
			...
			if e in links:
				...
				if len(nn)==0:
					...
					print('entity not in links',e,is_train,self.is_known,order)
					sys.exit(1)
			else:
				...
				if len(nn)==0:
					print('something wrong @ modelS')
					print('entity not in edges',e,is_train,self.is_known,order)
					sys.exit(1)
			for k in nn:
		...
		for k,v in sorted(neighbor_dict.items(),key=lambda x:x[1]):
			neighbor.append(k)
		x = self.get_context(neighbor,links,relations,edges,order+1,xp,is_train)
		x = getattr(self,self.forwardB[order][0])(x,neighbor,neighbor_dict,assign,entities,relations,is_train)
		return x

	def __call__(self,positive,negative,links,relations,edges,xp,is_train):
		...
		x = self.get_context(entities,links,relations,edges,0,xp,is_train)
	...
	def get_margins(self,candidates,links,relations,edges,xp,mode):
		...
		for h,r,t,l in candidates:
			...
			entities.add(t)
		entities = list(entities)
		xe = self.get_context(entities,links,relations,edges,0,xp,is_train)
